...
//CODE HERE
let greetUser = (u) => `Welcome back, ${u}`
console.log(greetUser('bigboi68'))
...
//CODE HERE
function canWeDeliver(z){
    if(deliveryAreaZipCodes.includes(z)){
        console.log(`${z} is in our delivery zone`)
    }else{
        console.log(`Sorry, we can't deliver to the ${z} delivery zone`)
    }
}
canWeDeliver(85205)
...
// CODE HERE
function canWeDeliverTwo(z){
 for(let i = 0; i < deliveryAreaZipCodes.length; i++){
    if(deliveryAreaZipCodes[i] === z){
        return `${z} is in our delivery zone`
    }

 }
 return `Sorry, we can't deliver to the ${z} delivery zone`
}
console.log(canWeDeliverTwo(85205))
console.log(canWeDeliverTwo(76509))
...
//CODE HERE
let salesChange = deals[0].title.replace('15','10' )
...
//CODE HERE
let saleExtention = deals[1].desc.replace('March', 'April').trim();
console.log(saleExtention)